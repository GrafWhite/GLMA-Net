...
# FusionIO_1 = FusionModule().cuda('cuda:0')
EdgeEnhanceinput = EdgeEnhancementModule()
afterFusion = MyAttention(2,4)


class SnakeDecoderBlock(nn.Module):
    def __init__(self, input_channel, output_channel, extend_scope = 1, if_offset = True, device = "cuda:1"):
...
class DecoderConv(nn.Module):
    def __init__(self, in_ch, out_ch, device='cuda:1'):
        super(DecoderConv, self).__init__()
        self.conv = nn.Conv2d(in_ch, out_ch, 3, padding=1).to(device)
        self.gn = nn.GroupNorm(out_ch // 4, out_ch).to(device)
        self.relu = nn.ReLU(inplace=True)
...
class EncoderConv(nn.Module):
    def __init__(self, in_ch, out_ch):
        ...
        self.gn = nn.GroupNorm(out_ch // 4, out_ch).cuda()
